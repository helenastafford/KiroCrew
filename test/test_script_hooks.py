...
class TestRunScriptHookSpawnForm:
    ...
    async def test_command_reaches_the_shell_verbatim(self, monkeypatch):
        ...
        # Both isolation wrappers forced to no-ops so the argv assertion below
        # sees the bare shell invocation. Without this the test only passes where
        # NO backend exists: a host with the Linux namespace sandbox (the
        # dedicated CI job) or cgroup v2 prepends its launcher to argv[0]. What
        # is under test is quote preservation, not which wrapper ran.
        monkeypatch.setattr("kiro_crew.sandbox.wrap_argv", lambda argv, **k: (list(argv), None))
        monkeypatch.setattr("kiro_crew.sandbox.cgroup_scope_argv", lambda argv: list(argv))
