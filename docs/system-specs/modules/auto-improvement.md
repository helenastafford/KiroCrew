...
legitimate offline setup. ARCC's guidance on untrusted URL destinations asks for exactly this —
allowlist the destination rather than trusting persisted input.
...
them, matching the ARCC input-validation guidance: allowlist at the point of origin, block
...
of granting ``tools`` but deliberately not ``allowedTools``. ARCC's GenAI tool-use guidance
