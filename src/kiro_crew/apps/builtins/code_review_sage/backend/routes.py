...
async def _handle_namespaces(request: web.Request) -> web.Response:
    ...
    if request.method == "DELETE":
        # A consolidation captured THIS namespace by name before it was dispatched,
        # so pruning the active list below does nothing for it: the worker would
        # finish later and `consolidate_apply` would recreate the directory and
        # ruleset after the caller was told the delete succeeded. Refuse up front,
        # before the active-list prune, so a rejected delete has no side effect.
        ...
        # Re-check with no await before the destructive call: the prune above is an
        # await, and a consolidation could have claimed the namespace during it.
        # Same reasoning as the claim in the consolidate handler — on the single
        # event loop a check with no await before the act it guards is atomic.
        if name in _CONSOLIDATING:
            return web.json_response(
                {"code": "consolidation_in_progress",
                 "error": "a consolidation started for this namespace — "
                          "wait for it to finish, then delete"},
                status=409)
