...
import { needsDeliveryRetry, retryActionFor } from './delivery'
import type {
    ...
    <div className="mb-0.5">
      ...
      <FolderBody open={open}>
        <div className="border-l border-border mb-1 ml-3 pl-1 rounded-bl-md">
          ...
          {/* ---- retry bar: same slot and shape as the send bar above, for a
                 request that was SEALED but never acknowledged. `/send` seals
                 server-side and the panel dispatches afterwards, so a tab closed
                 between the two leaves a sealed request the agent never saw —
                 and the send bar is already gone by then. Drawn in the danger
                 colour, not the accent, because it means something did not
                 complete rather than something is ready to go. ---- */}
          {!done && !isDraft && needsDeliveryRetry(req) && comments.length > 0 && (
            <div>
              ...
              >
                ...
                {sending
                  ? i18nT('apps.designTweak.requests.sending')
                  : i18nT('apps.designTweak.requests.resend_request', { number: req.number })}
              </button>
            </div>
          )}
        </div>
...
export default function DesignTweak() {
  ...
  // Requests whose prompt reached the agent but whose acknowledgement POST did
  // not. They must never be re-dispatched — only re-acked. A ref, not state:
  // nothing renders off it, and it must be readable at call time by the callback
  // below without re-creating it.
  ...
  const deliverSealed = useCallback(async (snap: Request, req: Request) => {
    ...
    try {
      if (key) {
        ...
        // Only a real session dispatch counts as delivered. The composer path
        // below hands off to the USER, who still has to press send — acking that
        // would clear the retry bar while the agent may never see the batch.
        //
        // If the ACK fails after a successful dispatch, the batch IS with the
        // agent — so remember that here. Without this, the retry bar stays up on
        // a request the agent already has, and pressing it dispatches a second
        // copy, applying every edit twice. `resendRequest` reads this set and
        // re-acks instead of re-dispatching.
        try {
          await markDelivered(req.id)
          deliveredRef.current.delete(req.id)
        } catch {
          deliveredRef.current.add(req.id)
        }
  ...
  }, [projects, previewId, ensureSlot, dispatch, navigate])
  ...
  // Retry a request that was sealed but never acknowledged — the tab closed (or
  // the dispatch threw) between `/send` and the prompt reaching the agent.
  const resendRequest = useCallback(async (req: Request) => {
    setSendingId(req.id)
    try {
      if (retryActionFor(req, deliveredRef.current) === 'reack') {
        // Dispatch already succeeded this session; only the ack is outstanding.
        // Re-dispatching would hand the agent a second copy of the same batch.
        await markDelivered(req.id)
        deliveredRef.current.delete(req.id)
      } else {
