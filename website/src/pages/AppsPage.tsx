...
export default function AppsPage() {
  ...
  const enableApp = async (name: string) => {
    ...
    try {
      ...
      else setError((e as Error)?.message || i18nT('pages.appsPage.could_not_enable_app', { name }))
    } finally {
  ...
  const handleAction = async (name: string, action: 'enable' | 'disable' | 'uninstall' | 'update') => {
    ...
    try {
      ...
      else setError((e as Error)?.message || i18nT('pages.appsPage.app_action_failed', { name }))
    } finally {
      setActionLoading(null)
    }
  }
